...
router.get(
  ...
  async (req, res) => {
    try {
      console.log("[GMB Sync] Manual sync requested by admin:", req.user.email, req.user._id.toString());

      const token = await getTokenForUser(req.user);
      if (!token) {
        console.warn("[GMB Sync] Manual sync blocked. No token for admin:", req.user.email, req.user._id.toString());
        return res.status(401).json({
          success: false,
          message: "Failed to sync GMB: authentication error",
          requiresReauth: true,
        });
      }

      const result = await syncGMBReviews({
        adminUser: req.user,
        trigger: "manual",
      });

      console.log("[GMB Sync] Manual sync finished for admin:", req.user.email, req.user._id.toString());
      res.status(200).json({
        success: true,
        message: result.message || "GMB review sync started successfully",
        processedCount: result.processedCount || 0,
      });
    } catch (error) {
      const statusCode = error.statusCode || 500;
      console.error("[GMB Sync] Manual sync failed for admin:", req.user.email, req.user._id.toString(), error.message);

      res.status(statusCode).json({
        success: false,
        message:
          error.code === "GMB_AUTH_ERROR"
            ? "Failed to sync GMB: authentication error"
